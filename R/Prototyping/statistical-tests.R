...
get_plots_pvalues <- function(input_res_DT) {
  ...
  for(dataset_i in res_DT[[2]][, unique(Dataset)]){
    i <- i + 1
    mean_DT <- res_DT[[2]][Dataset == dataset_i][, .SD[sample(1:.N, 10, replace = T), median(V2)], by = .(`Cross-Validation`, Representation)]
    original_DT <<- res_DT[[3]][Dataset == dataset_i]
    #original_DT1 <- original_DT[, .(`Cross-Validation`, Representation, V2)]
    original_DT1 <- original_DT[, .SD[sample(1:.N, 10, replace = T), median(V2)], by = .(`Cross-Validation`)]
  ...
  for(dataset_i in res_DT[[2]][, unique(Dataset)]){
    ...
    quantiles <- temp[, .SD[1:.N, quantile(V1, probs = 0.95)], by = .(`Cross-Validation`, Representation, features_Iteration)]
    
    merged_1randomDT <- temp[quantiles, on = c("Cross-Validation", "Representation", "features_Iteration")]
    DT_hyper <- merged_1randomDT[V1 >= i.V1, .SD[sample(1:.N, 5, replace = T), median(V2)], by = .(`Cross-Validation`, Representation, features_Iteration)]
    DT_hyper_final <- DT_hyper[, median(V1), by = .(`Cross-Validation`, Representation)]
    
    
    original_DT <- res_DT[[3]][Dataset == dataset_i]
    quantiles_original <- original_DT[, .SD[1:.N, quantile(V1, probs = 0.95)], by = .(`Cross-Validation`)]
    merged_originalDT <- original_DT[quantiles_original, on = c("Cross-Validation")]
    DT_hyper_original <- merged_originalDT[V1 >= i.V1, .SD[sample(1:.N, 5, replace = T), median(V2)], by = .(`Cross-Validation`)]
