...
create_unsupervised_scoresDT <- function(datasetname, percentage_OD, mixed_view_features) {
  ...
  if("FastABOD" %in% DT[, unique(OD)]){
    ...
    FastABOD_DT[, FastABOD:=1-FastABOD]
  } else{
    FastABOD_DT <- NULL
  }
  ...
  dimension <- dim(DToutliers_all)[2]
  
  
  
  mean_values <- data.table::transpose(DToutliers_all[, lapply(.SD, function(x) mean(x)), .SDcols = 1:dimension])
  mean_squared_values <- data.table::transpose(DToutliers_all[, lapply(.SD, function(x) (mean(x)^2)), .SDcols = 1:dimension])
  squared_mean_values <- data.table::transpose(DToutliers_all[, lapply(.SD, function(x) mean(x^2)), .SDcols = 1:dimension])
  ...
  DToutliers_all_gauss <- dplyr::bind_cols(temp_list)
  return(mixed_arthur = DToutliers_all_gauss)
}
